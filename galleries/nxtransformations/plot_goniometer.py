...
The figure shows the sample axes in the laboratory frame for the values above.
"""
...
sam_x = translation([1, 0, 0], 2)
sam_y = translation([0, 1, 0], 1)
sam_z = translation([0, 0, 1], 2)
omega = rotation(omega_axis, omega_angle)

sample_to_lab = omega @ sam_z @ sam_y @ sam_x @ chi @ phi

# A rotation acts in the frame of the transformations below it in the chain
downstream = {
...
# Laboratory frame at the origin, sample frame after the transformation chain
...
for index, (label, color) in enumerate(zip(AXIS_LABELS, AXIS_COLORS)):
    ...
    ax.quiver(
        0, 0, 0, *to_view(direction * 4)[0], color="silver", arrow_length_ratio=0.08
    )
    ax.text(*to_view(direction * 4.5)[0], label, color="gray")
    ax.quiver(
        *to_view(sample)[0],
        *to_view(sample_to_lab[:3, :3] @ direction * 2.0)[0],
        color=color,
        arrow_length_ratio=0.15,
        label=f"sample {label}",
    )
ax.plot(
    *to_view(np.vstack([ORIGIN, sample])).T, color="black", linestyle=":", linewidth=1
)
ax.scatter(*to_view(sample)[0], color="black", s=25, label="sample")

# Each rotation is drawn as a curved arrow around its own axis
for name, axis, angle, offset, color in [
    ("phi", phi_axis, phi_angle, 1.8, "tab:purple"),
    ("chi", chi_axis, chi_angle, 1.8, "tab:brown"),
    ("omega", omega_axis, omega_angle, 3.6, "tab:olive"),
]:
    frame = downstream[name]
    direction = np.asarray(axis, float)
    direction = direction / np.linalg.norm(direction)
    reference = [0, 1, 0] if abs(direction[2]) > 0.9 else [0, 0, 1]
    start = np.cross(direction, reference)
    start = start / np.linalg.norm(start) * 1.2

    line = to_lab(frame, np.array([[0, 0, 0], direction * (offset + 0.8)]))
    ax.plot(*to_view(line).T, color=color, linestyle="--", linewidth=1.0)
    points = to_lab(frame, arc(axis, angle, start) + direction * offset)
    ax.plot(*to_view(points).T, color=color, linewidth=1.6)
    ...
    label = to_lab(frame, np.array([direction * (offset + 1.9)]))
    ax.text(
        *to_view(label)[0],
        f"{name}={angle}\N{DEGREE SIGN}",
        color=color,
        fontsize="small",
        ha="center",
    )
